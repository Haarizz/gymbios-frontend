...
export default function CreatePlanPage() {
  ...
  const [form, setForm] = useState({
    ...
    category: "Membership",
    type: "Individual",
    durationType: "Monthly",    // Monthly / Weekly / Yearly / Single Session etc.
    durationValue: "",          // e.g. 1, 3, 12 months
    price: "",
    discount: "",
    ...
    accessTraining: TRAINING_STREAMS.reduce((acc, cur) => {
      acc[cur.id] = true;  // default all checked
      return acc;
    }, {}),
    accessFacilities: FACILITIES.reduce((acc, cur) => {
      acc[cur.id] = false; // default none selected
      return acc;
    }, {}),
  ...
  const toggleTrainingStream = (id) => {
    setForm((s) => ({ ...s, accessTraining: { ...s.accessTraining, [id]: !s.accessTraining[id] } }));
  };

  const toggleFacility = (id) => {
    setForm((s) => ({ ...s, accessFacilities: { ...s.accessFacilities, [id]: !s.accessFacilities[id] } }));
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    // Validate required fields (note: changed to durationValue)
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-6 max-w-4xl mx-auto">
        ...
        <div className="lg:hidden p-4 bg-white shadow flex items-center mb-6 rounded">
          ...
          />
          <h1 className="ml-4 font-bold text-lg">Create New Plan</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-600">Fill in the details to create a new membership plan.</p>
          </div>
          ...
          <section className="p-4 border rounded bg-gray-50 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {/* Optionally: expand/collapse toggler */}
            </div>
            <p className="text-gray-700 text-sm">Set program and class access for the membership.</p>

            <div className="grid grid-cols-12 gap-3 items-center">

              <div className="col-span-4">
                <label className="block text-xs font-bold mb-1">HOW MANY TIMES CAN A MEMBER ATTEND?</label>
                <select
              ...
              <div className="col-span-8">
                <label className="inline-flex items-center gap-2 text-xs font-semibold mb-1 opacity-70 cursor-default" title="Maximum number of members allowed for this membership.">
              ...
              <div className="col-span-12">
                <div className="flex justify-between mb-2">
                  ...
                  <div className="space-x-2 text-xs">
                    <button type="button" onClick={selectAllTraining} className="text-teal-600 hover:underline">
                      Select All
                    </button>
                    <button type="button" onClick={deselectAllTraining} className="text-teal-600 hover:underline">
                      Deselect All
                    </button>
                  </div>
                </div>
                ...
                <p className="mt-1 text-gray-600 text-xs">
                  {Object.values(form.accessTraining).filter(Boolean).length} of {TRAINING_STREAMS.length} training streams selected
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="p-4 border rounded bg-gray-50 space-y-4">
            ...
            <p className="text-gray-700 text-sm">Select which facilities members with this plan can access (only active facilities are shown).</p>

            <div className="flex items-center justify-between mb-2">
              <div>
                <button type="button" onClick={selectAllFacilities} className="text-teal-600 hover:underline text-xs mr-2">
                  Select All
                </button>
                <button type="button" onClick={deselectAllFacilities} className="text-teal-600 hover:underline text-xs">
                  Deselect All
                </button>
              </div>
              <div className="text-xs text-gray-600">
                {Object.values(form.accessFacilities).filter(Boolean).length} of {FACILITIES.length} selected
              </div>
            </div>
