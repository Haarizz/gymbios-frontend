import { ArrowTrendingUpIcon, ArrowTrendingDownIcon } from "@heroicons/react/24/solid"; // Using correct, reliable icon names from 24px solid

export default function ReportStatCard({ title, value, trend }) {
  // Default values to match the UI screenshot style if props aren't provided
  const trendValue = trend?.value || "+2.1%";
  const isPositive = trend?.isPositive !== false;
  const trendText = trend?.text || "from last month";
